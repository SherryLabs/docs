---
title: "sherry-sdk"
description: "TypeScript SDK for building mini-app metadata — define actions, validate schemas, and create triggers for social embedding"
sidebarTitle: "sherry-sdk"
---

The sherry-sdk (`@sherrylinks/sdk`) is the TypeScript library for building mini-app metadata. It defines the action schema, validates metadata before submission, and provides template helpers for common parameter types. Use it to create the metadata object that powers a Relayer Trigger.
...
- Mini-app metadata is a typed `Metadata` object with `url`, `icon`, `title`, `description`, and an `actions` array.
...
Use the [Sherry Debugger](https://app.sherry.social/debugger) to test and preview mini-app metadata before embedding. Paste your metadata JSON or URL and see the rendered Trigger in real time.
...
<CardGroup cols={2}>
  ...
  <Card title="Action Kit" icon="bolt" href="/action/overview">
    Register and publish mini-apps via the Relayer API.
  </Card>
</CardGroup>
