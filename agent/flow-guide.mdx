...
If the flow is interrupted between rounds 2 and 3, resume with `POST /v1/agents/{id}/prepare-policies`. Abandon with `DELETE /v1/agents/{id}` while status is `pending_policies`.
...
const data = await sdk.x402fetch("https://skill.example/api/run", {
  method: "POST",
