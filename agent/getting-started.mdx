...
<Steps>
  ...
  <Step title="3. Activate the Agent Kit module">
    ...
    - Enables the `/v1/agents/*` and `/v1/action/skills/*` endpoints for your workspace
  ...
  <Step title="5. Create your operator wallet (if you don't have one)">
    The agent's USDC funds come from your **operator wallet** — a self-custodial wallet your workspace owns. Create it via the Signing Kit prepare/confirm flow, or from the dashboard's **Wallets** tab.
  ...
  <Step title="6. Provision your first agent (3-round flow)">
    From the dashboard → **Agents → New agent**, or via API:

    1. `POST /v1/agents/prepare` — your backend calls this; returns an unsigned activity
    2. Dashboard prompts you to stamp the activity with your passkey (browser-side WebAuthn)
    3. `POST /v1/agents/confirm-user` — backend submits the stamped activity, gets back the `agentId` and a second unsigned activity
    4. Dashboard stamps again with passkey
    5. `POST /v1/agents/confirm-policies` — backend submits, receives the **`agentSecret`**

    <Warning>
      The `agentSecret` is shown **once**. Push it directly to your agent's secret store. There is no recovery — if lost, you must rotate via `POST /v1/agents/{id}/rotate`.
    </Warning>
  ...
  <Step title="8. Fund the agent wallet">
    Move USDC from your operator wallet to the agent wallet via the 2-step fund flow:

    ```text
    POST /v1/agents/{id}/fund/prepare  { amountUSDC: "100" }
    → returns unsigned activity for passkey stamping
    POST /v1/agents/{id}/fund/confirm  { stampedActivity: "..." }
    → broadcasts the transfer to Solana
    ```

    Or do it from the dashboard's agent detail view. Either way, the agent now has USDC and can spend up to its `payments` budget.
  </Step>
  ...
  <Step title="10. Configure environment variables">
    ...
    # RELAYER_API_URL=https://relayer-api.onrender.com
    ```
  ...
  <Step title="12. Make your agent spend money">
    The simplest paid action is paying for an x402-gated skill:
...
<CardGroup cols={2}>
  ...
  <Card title="Skills marketplace" icon="cube" href="/api-reference/introduction">
    Discover and execute paid skills. Agents pay via x402, the SDK handles it transparently.
  </Card>
